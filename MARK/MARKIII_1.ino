...
void setup()
{
  ...
  Serial.begin(115200);
  mcp.begin();
  mcp.pinMode(LEDPINRED, OUTPUT);
  mcp.digitalWrite(LEDPINRED, HIGH);
  mcp.pinMode(LEDPINBLUE, OUTPUT);
  mcp.digitalWrite(LEDPINBLUE, HIGH);
  mcp.pinMode(RELAYPIN, OUTPUT);
  mcp.digitalWrite(RELAYPIN, LOW);
  mcp.pinMode(BUZZERPIN, OUTPUT);
  mcp.digitalWrite(BUZZERPIN, HIGH);
  mcp.pinMode(MAGNETICPIN, INPUT);
  mcp.pullUp(MAGNETICPIN, HIGH);
...
void loop()
{
  ...
  {
    LINE.notify(String(settings.namedevice) + " DOOR CLOSE");
    case1 = false;
  }
